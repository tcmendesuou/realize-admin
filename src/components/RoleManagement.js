  ...
  return (
    <>
      <style>{`
        ...
        .rm-panel-header { padding: 14px 16px; border-bottom: 1px solid #f0f2f5; display: flex; align-items: center; justify-content: space-between; background: #fafbfc; flex-shrink: 0; }
        .rm-panel-title { font-size: 11px; font-weight: 600; letter-spacing: 1.5px; text-transform: uppercase; color: #8a9bb0; }
        .rm-panel-add { width: 24px; height: 24px; border-radius: 6px; border: 1px solid #e0e0e0; background: none; cursor: pointer; font-size: 16px; color: #667eea; display: flex; align-items: center; justify-content: center; line-height: 1; transition: all 0.15s; }
        ...
        .rm-item { display: flex; align-items: center; justify-content: space-between; padding: 10px 12px; border-radius: 8px; cursor: pointer; transition: all 0.15s; margin-bottom: 2px; }
        .rm-item:hover { background: #f5f5f5; }
        .rm-item.active { background: #f0f3ff; }
        .rm-item-info { flex: 1; min-width: 0; }
        .rm-item-name { font-size: 13px; color: #2c3e50; font-weight: 400; display: block; }
        .rm-item.active .rm-item-name { color: #667eea; font-weight: 500; }
        .rm-item-systemrole { font-size: 10px; color: #aaa; display: block; margin-top: 2px; }
        .rm-item-arrow { font-size: 10px; color: #ccc; flex-shrink: 0; margin: 0 4px; }
        ...
        .rm-modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 1000; display: flex; align-items: center; justify-content: center; }
        .rm-modal { background: white; border-radius: 12px; padding: 28px; width: 380px; box-shadow: 0 10px 40px rgba(0,0,0,0.15); }
        ...
        .rm-modal-cancel { flex: 1; padding: 10px; border: 1px solid #e0e0e0; border-radius: 8px; background: none; color: #777; font-family: 'Outfit', sans-serif; font-size: 14px; cursor: pointer; }
        .rm-modal-confirm { flex: 2; padding: 10px; border: none; border-radius: 8px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; font-family: 'Outfit', sans-serif; font-size: 14px; font-weight: 500; cursor: pointer; }
        ...
        <div className="rm-cascade">
          ...
          <div className="rm-panel">
            <div className="rm-panel-header">
              ...
              <button className="rm-panel-add" title="Nova área" disabled={!selectedType} onClick={() => setShowAreaModal(true)}>+</button>
            </div>
            <div className="rm-panel-list">
              {!selectedType ? <div className="rm-empty">Selecione um tipo</div>
                : filteredAreas.length === 0 ? <div className="rm-empty">Nenhuma área</div>
                : filteredAreas.map(a => (
                  <div key={a.id}
          ...
          <div className="rm-panel">
            <div className="rm-panel-header">
              ...
              <button className="rm-panel-add" title="Novo cargo" disabled={!selectedArea} onClick={() => setShowRoleModal(true)}>+</button>
            </div>
            <div className="rm-panel-list">
              {!selectedArea ? <div className="rm-empty">Selecione uma área</div>
                : filteredRoles.length === 0 ? <div className="rm-empty">Nenhum cargo</div>
                : filteredRoles.map(r => (
                  <div key={r.id}
        ...
        <div className="rm-modal-overlay" onClick={() => setShowTypeModal(false)}>
          <div className="rm-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="rm-modal-field">
              ...
              <p className="rm-modal-hint">Define qual tela este tipo de usuário vai acessar ao fazer login.</p>
            </div>
