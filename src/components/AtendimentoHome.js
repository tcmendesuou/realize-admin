...
export default function AtendimentoHome({ user, userData, onLogout }) {
  ...
  const loadCompaniesAndEventTypes = async () => {
    try {
      const [compSnap, etSnap] = await Promise.all([
        getDocs(query(collection(db, 'companies'), where('type', '==', 'cliente'))),
  ...
  const loadFlowQuestions = async (eventTypeId) => {
    try {
      ...
      const questionIds = questionItems.map(i => i.itemId);
  ...
  const renderQuestionInput = (q) => {
    ...
    const base = {
      width: '100%', padding: '10px 14px', borderRadius: 8, border: '1px solid rgba(0,180,255,0.15)',
      background: 'rgba(255,255,255,0.04)', color: '#E8F4FF', fontFamily: 'Outfit, sans-serif',
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {['Sim', 'Não'].map(opt => (
            <button key={opt} onClick={() => handleAnswerChange(q.id, opt)}
              style={{ ...base, width: 'auto', padding: '8px 20px', cursor: 'pointer',
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .at-btn-briefing-icon { font-size: 16px; line-height: 1; }
        ...
        <aside className="at-sidebar">
          ...
          <nav className="at-nav">
            ...
            <button className="at-nav-item" style={{ opacity: 0.4, cursor: 'not-allowed' }}>
              <span className="at-nav-dot" /><span>Detalhes do Projeto</span>
            </button>
            <button className="at-nav-item" style={{ opacity: 0.4, cursor: 'not-allowed' }}>
        ...
        <main className="at-main">
          ...
          <div className="at-header">
            ...
            <button className="at-btn-briefing" onClick={() => setShowBriefing(true)}>
              <span className="at-btn-briefing-icon">+</span>
              Abrir novo briefing
            </button>
          </div>
